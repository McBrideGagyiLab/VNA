...
%%%%%%%%%%%%% scan along XY %%%%%%%%%%%%%%%%
WAIT_X = waitbar(1/shape(3),'Scan Along XY...');
for k=1:shape(3)
    waitbar(k/shape(3),WAIT_X,'Scan along XY...');
    ...
    if(exist('bwdist') && aspect(1)==aspect(2))
        ...
        WAIT_Y = waitbar(1/shape(3),'Scan Along Y...');
        for i=1:shape(2)
            waitbar(i/shape(2),WAIT_Y,'Scan along Y...');
...
WAIT_E = waitbar(1/shape(3), 'Build envelope...');
for k=1:shape(3)
    waitbar(k/shape(3), WAIT_E, 'Build envelope...');
